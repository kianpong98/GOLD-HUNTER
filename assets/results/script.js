...
async function scanContinuousAfterManifest(folder,prefix,items,allowedExtensions=MEDIA_EXTENSIONS){
  const found=[];
  const existing=new Set(items.map(x=>x.src));
  let highest=items.reduce((m,x)=>Math.max(m,mediaNumber(x.src,prefix)),0);
  let misses=0;
  for(let i=highest+1;i<=highest+40;i++){
    const item=await findMediaByNumber(folder,prefix,i,allowedExtensions);
    if(item && !existing.has(item.src)){
      found.push(item);
...
async function loadStableGallery(folder,prefix,allowedExtensions=MEDIA_EXTENSIONS){
  const manifestItems=await loadManifest(folder);
  let items=manifestItems;
  // One-time scan only. No background re-render, so images will not disappear later.
  const extra=await scanContinuousAfterManifest(folder,prefix,items,allowedExtensions);
  items=uniqueBySrc([...items,...extra]);
  if(!items.length){
    // Fallback if manifest is missing: read continuous sequence from 001.
    const fallback=[];
    let misses=0;
    for(let i=1;i<=80;i++){
      const item=await findMediaByNumber(folder,prefix,i,allowedExtensions);
  ...
  return sortLatestFirst(items,prefix);
}
