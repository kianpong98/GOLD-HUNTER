(()=>{
  ...
  function values(e){
    ...
    const impactText=String(e.goldImpact||'').toLowerCase();
    const impactClass=impactText.includes('supportive')||impactText.includes('bullish')?'gold-bullish':impactText.includes('negative')||impactText.includes('bearish')||impactText.includes('pressure')?'gold-bearish':impactText.includes('neutral')?'gold-neutral':'';
    const insight=(e.comparison||e.goldImpact)?`<div class="release-insight ${impactClass}"><span class="comparison ${String(e.comparison||'').toLowerCase().replace(/\s+/g,'-')}">${esc(e.comparison||'')}${e.comparisonZh?` · ${esc(e.comparisonZh)}`:''}</span>${e.difference?`<b>${esc(e.difference)}</b>`:''}${e.goldImpact?`<span class="gold-impact-label">${esc(e.goldImpact)}</span><small class="gold-impact-zh">${esc(e.goldImpactZh||'')}</small>`:''}${e.surpriseStrength?`<em>${esc(e.surpriseStrength)} · ${esc(e.surpriseStrengthZh||'')}</em>`:''}</div>`:'';
  ...
  function card(e,full=false){
    ...
    const header=`<div class="event-heading"><h3>${esc(e.name)}</h3><p class="event-zh">${esc(e.nameZh||'')}</p><p class="event-time">${esc(fmt(e.datetime))}</p><strong class="event-countdown" data-date="${esc(e.datetime)}">${esc(countdown(e.datetime))}</strong></div>`;
    return `<article class="${full?'calendar-item':'market-event-row'} ${e.released?'is-released':'is-scheduled'}" data-news-type="${esc(e.type||e.id||'economic_event')}" data-event-type="${esc(e.type||e.id||'economic_event')}" data-event-name="${esc(e.name||'Economic event')}" data-analytics-section="economic_news"><div class="${full?'calendar-date':'market-event-main'}">${header}</div>${full?`<div class="calendar-copy"><details><summary>为什么重要？</summary><p>${esc(e.whyZh||'此数据可能影响美元、利率预期与黄金波动。')}</p></details>${source}${historyTable(e)}</div>`:''}${values(e)}</article>`;
  }
  ...
  function sortCalendarEvents(events){
    const now=Date.now(),today=mytDayKey(now);
    const bucket=e=>{const key=mytDayKey(e.datetime),time=new Date(e.datetime).getTime();if(key===today)return 0;return time>=now?1:2;};
