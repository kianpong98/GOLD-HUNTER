(()=>{
  ...
  function values(e){
    ...
    const insight=(e.comparison||e.goldImpact)?`<div class="release-insight"><span class="comparison ${String(e.comparison||'').toLowerCase().replace(/\s+/g,'-')}">${esc(e.comparison||'')}</span>${e.difference?`<b>${esc(e.difference)}</b>`:''}${e.goldImpact?`<span>${esc(e.goldImpact)}</span><small>${esc(e.goldImpactZh||'')}</small>`:''}</div>`:'';
  ...
  function historyTable(e){if(e.eventOnly)return'';const rows=Array.isArray(e.history)?e.history.slice(0,10):[];if(!rows.length)return'';return `<details class="release-history"><summary>Past releases (${rows.length})</summary><div class="history-scroll"><table><thead><tr><th>Date / Period</th><th>Actual</th><th>Forecast</th><th>Previous</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${esc(r.dateTime||r.period||'—')}</td><td>${esc(r.actual||'—')}</td><td>${esc(r.forecast||'—')}</td><td>${esc(r.previous||'—')}</td></tr>`).join('')}</tbody></table></div></details>`;}
