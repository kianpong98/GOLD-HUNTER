(()=>{
  ...
  const FORECAST_API='/api/forecast-overrides';
  ...
  function values(e){
    ...
    const insight=(e.comparison||e.goldImpact)?`<div class="release-insight"><span class="comparison ${String(e.comparison||'').toLowerCase().replace(/\s+/g,'-')}">${esc(e.comparison||'')}</span>${e.difference?`<b>${esc(e.difference)}</b>`:''}${e.goldImpact?`<span>${esc(e.goldImpact)}</span><small>${esc(e.goldImpactZh||'')}</small>`:''}${e.surpriseStrength?`<em>${esc(e.surpriseStrength)} · ${esc(e.surpriseStrengthZh||'')}</em>`:''}</div>`:'';
  ...
  async function loadEvents(){const home=document.getElementById('homeMarketEvents'),full=document.getElementById('calendarList');if(!home&&!full)return;try{const eventUrl=`${EVENTS_API}?v=${Date.now()}`;const [r,fr]=await Promise.all([fetch(eventUrl,{cache:'no-store',headers:{'cache-control':'no-cache','pragma':'no-cache'}}),fetch(`${FORECAST_API}?v=${Date.now()}`,{cache:'no-store',headers:{'cache-control':'no-cache','pragma':'no-cache'}})]),d=await r.json(),fd=fr.ok?await fr.json():{overrides:{}};if(!r.ok)throw new Error(d.error||'Calendar unavailable');const overrides=fd.overrides||{};const merged=(d.events||[]).map(e=>{const keys=[String(e.id||''),`${String(e.type||'')}|${String(e.releasePeriod||'')}`,String(e.type||'')].filter(Boolean);for(const k of keys){if(Object.prototype.hasOwnProperty.call(overrides,k))return {...e,forecast:String(overrides[k]??'')};}return e;});const events=merged.filter(e=>Number(e.impact)>=4&&!/ism/i.test(String(e.type||''))&&!/ISM/i.test(String(e.name||''))).sort((a,b)=>new Date(a.datetime)-new Date(b.datetime));const relevant=events.filter(e=>new Date(e.datetime).getTime()>Date.now());if(home)home.innerHTML=relevant.slice(0,3).map(e=>card(e,false)).join('')||'<div class="market-empty">No upcoming events.</div>';if(full)full.innerHTML=events.map(e=>card(e,true)).join('')||'<div class="market-empty">No events.</div>';const stamp=document.getElementById('calendarUpdated');if(stamp)stamp.textContent=`Last updated: ${new Intl.DateTimeFormat('en-MY',{timeZone:'Asia/Kuala_Lumpur',dateStyle:'medium',timeStyle:'medium'}).format(new Date(d.updatedAt))} MYT`;updateCountdowns();}catch(e){const msg=`<div class="market-empty"><b>Calendar unavailable.</b><span>${esc(e.message)}</span></div>`;if(home)home.innerHTML=msg;if(full)full.innerHTML=msg;}}
