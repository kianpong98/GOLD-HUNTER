(()=>{
  ...
  function renderSessions(){
    ...
    if(strip){
      ...
      `<span class="session-strip-quote"><b>XAU</b><span id="stripGoldPrice">—</span></span><span class="session-strip-quote"><b>DXY</b><span id="stripDxyPrice">—</span></span>`;
    }
  ...
  function reactionCell(v){if(v===null||v===undefined||v==='')return'—';const n=Number(v);if(!Number.isFinite(n))return esc(v);return `<span class="reaction ${n>0?'up':n<0?'down':'flat'}">${n>0?'▲':n<0?'▼':'•'} ${n>0?'+':''}${n.toFixed(2)} USD</span>`;}
  function historyTable(e){const rows=Array.isArray(e.history)?e.history.slice(0,10):[];if(!rows.length)return'';return `<details class="release-history"><summary>Past releases (${rows.length})</summary><div class="history-scroll"><table><thead><tr><th>Date / Period</th><th>Actual</th><th>Forecast</th><th>Previous</th><th>5m</th><th>30m</th><th>1h</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${esc(r.dateTime||r.period||'—')}</td><td>${esc(r.actual||'—')}</td><td>${esc(r.forecast||'—')}</td><td>${esc(r.previous||'—')}</td><td>${reactionCell(r.gold5m)}</td><td>${reactionCell(r.gold30m)}</td><td>${reactionCell(r.gold1h)}</td></tr>`).join('')}</tbody></table></div></details>`;}
  ...
  function setQuote(id,value,change){const price=document.getElementById(id+'Price'),chg=document.getElementById(id+'Change'),strip=document.getElementById(id==='gold'?'stripGoldPrice':'stripDxyPrice');if(price)price.textContent=money(value);if(strip)strip.textContent=money(value);if(chg){const n=Number(change);chg.className=Number.isFinite(n)?(n>0?'up':n<0?'down':'flat'):'';chg.textContent=Number.isFinite(n)?`${n>0?'▲ +':n<0?'▼ ':'• '}${n.toFixed(2)}%`:'Delayed quote';}}
  async function loadSnapshot(){try{const r=await fetch(SNAPSHOT_API,{cache:'no-store'}),d=await r.json();if(!r.ok)throw new Error(d.error||'Quote unavailable');setQuote('gold',d.gold?.price,d.gold?.changePct);setQuote('dxy',d.dxy?.price,d.dxy?.changePct);const u=document.getElementById('marketSnapshotUpdated');if(u)u.textContent=d.updatedAt?`Updated ${new Intl.DateTimeFormat('en-MY',{timeZone:'Asia/Kuala_Lumpur',hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:false}).format(new Date(d.updatedAt))}`:'Delayed';}catch(e){const u=document.getElementById('marketSnapshotUpdated');if(u)u.textContent='Quote delayed';}}
