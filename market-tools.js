(()=>{
  ...
  function card(e,full=false){
    ...
    if(!full){
      const homeInsight=(e.comparison||e.goldImpact)?`<div class="home-impact ${tone}"><div class="home-impact-top"><strong>${esc(e.comparison||'')}${e.comparisonZh?` · ${esc(e.comparisonZh)}`:''}</strong>${e.difference?`<b>${esc(e.difference)}</b>`:''}</div>${e.goldImpact?`<div class="home-gold-impact"><span>${esc(e.goldImpactZh||e.goldImpact)}</span><small>${esc(e.goldImpact)}</small></div>`:''}${e.surpriseStrength?`<em>${esc(e.surpriseStrength)}${e.surpriseStrengthZh?` · ${esc(e.surpriseStrengthZh)}`:''}</em>`:''}</div>`:'';
      return `<article class="market-event-row home-news-card ${tone}" data-news-type="${esc(e.type||e.id||'economic_event')}" data-event-type="${esc(e.type||e.id||'economic_event')}" data-event-name="${esc(e.name||'Economic event')}" data-analytics-section="economic_news"><div class="home-event-info"><h3>${esc(e.name)}</h3><p class="event-zh">${esc(e.nameZh||'')}</p><p class="home-event-time">${esc(fmt(e.datetime))}</p><strong class="event-countdown" data-date="${esc(e.datetime)}">${esc(countdown(e.datetime))}</strong></div><div class="home-event-data"><div class="home-metrics"><div><small>Actual</small><b>${esc(e.actual||'—')}</b></div><div><small>Forecast</small><b>${esc(e.forecast||'—')}</b></div><div><small>Previous</small><b>${esc(e.previous||'—')}</b></div></div>${homeInsight}</div></article>`;
    }
