(()=>{
  ...
  async function loadFedEditor(){
    ...
    try{
      ...
      setValue('fedEffectiveSource',d.sourceMode==='manual-admin-primary'?'Admin Manual (primary)':d.sourceMode==='official-github-sync'?'CME Official':d.sourceMode==='cloudflare-cron-calculated'?'Gold Hunter Auto (calculated)':'Verified fallback');setValue('fedLastCmeCheck',fmt(d.cmeLastCheckedAt||d.lastCheckedAt||d.updatedAt));
      const badge=$('#fedEffectiveBadge');if(badge){badge.textContent=d.sourceMode==='manual-admin-primary'?'ADMIN PRIMARY':d.sourceMode==='official-github-sync'?'CME LIVE':d.sourceMode==='cloudflare-cron-calculated'?'AUTO CALCULATED':'FALLBACK';badge.className=`badge ${['manual-admin-primary','official-github-sync','cloudflare-cron-calculated'].includes(d.sourceMode)?'ok':'warn'}`;}
      st.textContent=d.sourceMode==='manual-admin-primary'?'你现在拥有完整控制权；网站正在使用这里保存的日期、时间、概率和来源。':d.sourceMode==='official-github-sync'?'手动优先已关闭，网站使用 CME 自动数据。':d.sourceMode==='cloudflare-cron-calculated'?'手动优先已关闭，网站使用 Gold Hunter 每5分钟自动计算的数据（基于公开期货价格，非CME官方数据）。':'当前使用最后验证快照。';
  ...
  async function loadAnalytics(){
    ...
    try{
      ...
      renderRank('aWaSources',wa.sources);renderRank('aButtons',d.topButtons);renderRank('aWaPages',wa.pages,r=>r.dimensions?.[1]||r.dimensions?.[0]||'Unknown');renderRank('aWaCampaigns',wa.campaigns,r=>{const v=r.dimensions?.[0]||'(not set)';return v==='(not set)'?'No UTM campaign':v;});renderRank('aSections',(d.topSections&&d.topSections.length?d.topSections:d.topSectionIds));renderRank('aExitSections',d.exitSections);
      const ev=d.analyticsHealth?.eventCounts||{},eventUsers=d.analyticsHealth?.eventUsers||{};const funnel=[['Visitors',seven.activeUsers],['Section viewers',eventUsers.section_view||0],['Engaged 5s+',eventUsers.engaged_time||0],['WhatsApp users',wa.sevenDayUsers||0]];const funnelEl=$('#aFunnel');if(funnelEl)funnelEl.innerHTML=funnel.map(([label,value])=>`<div class="wa-funnel-step"><span>${esc(label)}</span><b>${metric(value)}</b></div>`).join('');
      ...
      const labels={topSections:'page_section',topSectionIds:'section_id（可选）',topButtons:'button_location',topNewsNames:'news_name（建议建立）',topNewsTypes:'news_type',exitSections:'page_section / section_id'};
      const eventSummary=['section_view','exit_section','news_interest','whatsapp_click'].map(name=>`${name}: ${metric(eventCounts[name]||0)}`).join(' · ');
  ...
  async function loadDataHealth(){
    ...
    safe('dcCloudflare',fetched.every(x=>x.ok)?'● Online':fetched.some(x=>x.ok)?'▲ Partial':'● Offline');const news=by.news.data||{};safe('dcKv',news.kvConfigured===false?'▲ Missing':'● Bound');safe('dcForecast',news.kvConfigured===false?'▲ Unavailable':'● Ready');const fed=by.fed.data||{};safe('dcSourceMode',fed.sourceMode==='official-github-sync'?'CME Official':fed.sourceMode==='cloudflare-cron-calculated'?'Gold Hunter Auto':fed.sourceMode==='manual-admin-fallback'?'Fed Manual':'Mixed / Cached');safe('dcNewsFreshness',newsChecked?agoText(newsChecked):'No data');safe('dcEtfFreshness',etfChecked?agoText(etfChecked):'No data');safe('dcFedFreshness',fedChecked?agoText(fedChecked):'No data');safe('dcGoldFreshness',goldChecked?agoText(goldChecked):'No data');safe('dcStatusLine',`Last dashboard check: ${fmt(nowIso)} · Read-only health check; no extra KV writes.`);
  ...
  async function runSystemAudit(){
    ...
    try{
      ...
      if(!fed.ok)checks.push(['Fed Rate','FAIL',fed.error]);else{const d=fed.data||{},total=(d.outcomes||[]).reduce((s,x)=>s+Number(x.probability||0),0),mode=d.sourceMode||'unknown',valid=Math.abs(total-100)<=1&&(d.outcomes||[]).length>=2;const official=mode==='official-github-sync'&&d.live===true,calculated=mode==='cloudflare-cron-calculated'&&d.live===true,status=!valid?'FAIL':(official||calculated||mode==='manual-admin-primary')?'PASS':'WARNING';checks.push(['Fed Rate',status,`${mode} · probability total ${total.toFixed(1)}%${official?' · CME live':calculated?' · Gold Hunter auto-calculated':' · fallback/manual active'}`]);}
  ...
  async function runNewsRecovery(){
    ...
    try{
      ...
      st.textContent=`发现 ${before.missing.length} 条缺少 Actual，正在重新合并已部署的官方快照…`;
      ...
      const d=await r.json();
      if(!r.ok)throw new Error(d.error||d.message||'无法执行新闻重新合并');
      st.textContent=d.ready?'重新合并成功，网站已显示全部应发布 Actual。':`已完成立即重新合并；仍等待 ${(d.missing||[]).map(x=>x.name).join('、')}。后台 GitHub Watcher 会继续每5分钟自动抓取和自修复。`;
      for(let attempt=1;attempt<=18;attempt++){
        ...
        st.textContent=`自动验证中（${attempt}/18）：仍等待 ${(status.missing||[]).map(x=>x.name).join('、')}；无需保持 Admin 或电脑开启。`;
      }
      st.textContent='后台自动抓取与自修复仍在运行；无需保持 Admin 或电脑开启。稍后重新打开即可查看结果。';
