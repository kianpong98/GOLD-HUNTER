(()=>{
  ...
  $('#saveEvents').addEventListener('click',async()=>{collect();const st=$('#saveStatus'),btn=$('#saveEvents');st.textContent='保存中…';btn.disabled=true;try{const r=await fetch(`${API}?v=${Date.now()}`,{method:'POST',headers:{'content-type':'application/json','x-admin-pin':pin,'cache-control':'no-cache','pragma':'no-cache'},body:JSON.stringify({events})}),d=await r.json();if(!r.ok)throw new Error(d.error||'保存失败');st.textContent=`已保存 ${d.count} 项。Forecast 已写入实时数据，网站会立即重新读取。`;try{localStorage.setItem('gh-market-events-updated',String(Date.now()));}catch{}await load();}catch(e){st.textContent=e.message;}finally{btn.disabled=false;}});
