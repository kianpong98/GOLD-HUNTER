(()=>{
  const API='/api/market-events'; let pin='',events=[],meta={};
  ...
  function render(){
    const rows=visibleEvents(); $('#adminEvents').innerHTML=rows.length?rows.map(({e,i})=>{
      ...
      const fields=eventOnly?`<label class="label">Malaysia date & time<input class="input" data-k="datetime" type="datetime-local" value="${esc((e.datetime||'').slice(0,16))}"></label><label class="label">Status<input class="input locked" readonly value="${esc(e.status||'Scheduled')}"></label>`:`<label class="label">Malaysia date & time<input class="input" data-k="datetime" type="datetime-local" value="${esc((e.datetime||'').slice(0,16))}"></label><label class="label">Forecast<input class="input" data-k="forecast" value="${esc(e.forecast)}" placeholder="例如 2.8% / 180K"></label><label class="label">Previous<input class="input" data-k="previous" value="${esc(/pending|sync/i.test(e.previous||'')?'':e.previous)}" placeholder="官方自动；必要时可手动填"></label><label class="label">Actual (Official)<input class="input locked" readonly value="${esc(e.actual||'—')}"></label>`;
  ...
  function updateStats(){const waiting=events.filter(e=>e.previousStatus==='awaiting_official'||(e.released&&!e.actual&&!e.eventOnly)).length;$('#totalCount').textContent=events.length;$('#readyCount').textContent=events.filter(e=>e.officialAuto&&e.previousStatus==='ready').length;$('#waitingCount').textContent=waiting;$('#forecastCount').textContent=events.filter(e=>e.forecast).length;
    const s=meta.officialSources||{};const items=[['Static Cache',s.staticCache],['BLS',s.bls],['FRED',s.fred],['Cloudflare KV',meta.kvConfigured]];$('#sourceGrid').innerHTML=items.map(([n,v])=>`<div class="source ${v?'ok':'bad'}"><b>${v?'● Connected':'● Not connected'}</b>${n}</div>`).join('');$('#syncLine').textContent=`页面更新：${fmt(meta.updatedAt)} · 官方数据更新：${fmt(meta.officialUpdatedAt)}${meta.officialError?' · Error: '+meta.officialError:''}`;}
  const fmt=v=>v?new Date(v).toLocaleString('zh-CN',{timeZone:'Asia/Kuala_Lumpur',hour12:false}):'尚无记录';
  function collect(){document.querySelectorAll('.event').forEach(card=>{const i=+card.dataset.i;card.querySelectorAll('[data-k]').forEach(input=>{let v=input.value.trim();if(input.dataset.k==='datetime'&&v&&!/[zZ]|[+-]\d\d:\d\d$/.test(v))v+=':00+08:00';events[i][input.dataset.k]=v;});});}
  async function load(){const r=await fetch(API,{headers:{'x-admin-pin':pin},cache:'no-store'}),d=await r.json();if(!r.ok)throw new Error(d.error||'无法打开后台');events=d.events||[];meta=d;render();}
