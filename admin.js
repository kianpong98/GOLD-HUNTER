(()=>{
  ...
  function updateStats(){const waiting=events.filter(e=>e.previousStatus==='awaiting_official'||(e.released&&!e.actual&&!e.eventOnly)).length;$('#totalCount').textContent=events.length;$('#readyCount').textContent=events.filter(e=>e.officialAuto&&e.previousStatus==='ready').length;$('#waitingCount').textContent=waiting;$('#forecastCount').textContent=events.filter(e=>e.forecast).length;
    ...
    $('#sourceGrid').innerHTML=newsHealth.length?newsHealth.map(v=>{const status=effectiveStatus(v),recent=status==='live',hasError=Boolean(v.error);return `<div class="source ${recent?'ok':'bad'}"><b>${label[status]||label.offline}</b><strong class="source-news-name">${esc(v.name)}</strong><span class="source-provider">${esc(v.provider||'Official source')}</span><small>Last successful access: ${fmt(v.lastSuccess)}${v.lastSuccess?` · ${agoText(v.lastSuccess)}`:''}</small><small>Last poll: ${fmt(v.lastChecked)}</small><small>HTTP: ${v.httpStatus??'—'}${Number.isFinite(Number(v.latencyMs))?` · ${Number(v.latencyMs)} ms`:''}</small><small>Last data change: ${fmt(v.lastDataChanged)}</small>${status==='cached'?'<small class="source-recovery">Verified cache available · retrying automatically</small>':''}${hasError?`<details class="source-error-details"><summary>Technical details</summary><small class="source-error">${esc(shortError(v.error))}</small><small class="source-error-full">${esc(v.error)}</small></details>`:''}<small class="source-recovery">${esc(v.recovery||'')}</small></div>`;}).join(''):items.map(([n,v])=>{const status=effectiveStatus(v);return `<div class="source ${status==='live'?'ok':'bad'}"><b>${label[status]||label.offline}</b>${n}<small>Last checked: ${fmt(v.lastChecked||meta.lastCheckedAt||meta.updatedAt)}</small><small>HTTP: ${v.httpStatus??'—'}${Number.isFinite(Number(v.latencyMs))?` · ${Number(v.latencyMs)} ms`:''}</small><small>Last data change: ${fmt(v.lastSuccess)}</small></div>`;}).join('');
