(()=>{
  ...
  function updateStats(){const waiting=events.filter(e=>e.previousStatus==='awaiting_official'||(e.released&&!e.actual&&!e.eventOnly)).length;$('#totalCount').textContent=events.length;$('#readyCount').textContent=events.filter(e=>e.officialAuto&&e.previousStatus==='ready').length;$('#waitingCount').textContent=waiting;$('#forecastCount').textContent=events.filter(e=>e.forecast).length;
    ...
    const technicalDetails=(v,status)=>{if(!v.error)return '';const summary=status==='live'?'Previous retry details':'Technical details';return `<details class="source-error-details"><summary>${summary}</summary><small class="source-error">${esc(shortError(v.error))}</small><small class="source-error-full">${esc(v.error)}</small></details>`;};
    const httpLine=v=>{const code=Number(v.httpStatus);const latency=Number(v.latencyMs);if(!code&&!Number.isFinite(latency))return '';return `<small>HTTP${code?` ${code}`:''}${Number.isFinite(latency)?` · ${latency} ms`:''}</small>`;};
    const newsHealth=Array.isArray(meta.connectionHealth)?meta.connectionHealth:[];
    $('#sourceGrid').innerHTML=newsHealth.length?newsHealth.map(v=>{const status=effectiveStatus(v),recent=status==='live';return `<div class="source ${recent?'ok':status==='cached'?'recovering':'bad'}"><b>${label[status]||label.offline}</b><strong class="source-news-name">${esc(v.name)}</strong><span class="source-provider">${esc(v.provider||'Official source')}</span><small>Last successful: <b>${v.lastSuccess?agoText(v.lastSuccess):'No successful check'}</b></small>${v.lastSuccess?`<small>${fmt(v.lastSuccess)}</small>`:''}<small>Last poll: ${fmt(v.lastChecked)}</small>${httpLine(v)}<small>Last data change: ${fmt(v.lastDataChanged)}</small>${status==='cached'?'<small class="source-recovery">Verified cache active · automatic recovery running</small>':''}${technicalDetails(v,status)}${v.recovery?`<small class="source-recovery">${esc(v.recovery)}</small>`:''}</div>`;}).join(''):items.map(([n,v])=>{const status=effectiveStatus(v);return `<div class="source ${status==='live'?'ok':status==='cached'?'recovering':'bad'}"><b>${label[status]||label.offline}</b><strong class="source-news-name">${esc(n)}</strong><small>Last successful: ${v.lastSuccess?agoText(v.lastSuccess):'No record'}</small>${httpLine(v)}<small>Last data change: ${fmt(v.lastSuccess)}</small></div>`;}).join('');
  ...
  function updateSyncMonitor(items){
    ...
    const latest=latestSuccess(items), statuses=items.map(([,v])=>effectiveStatus(v)), nonLive=statuses.filter(v=>v!=='live').length;
    const points=statuses.map(v=>v==='live'?100:v==='cached'?80:0),health=Math.round(points.reduce((a,b)=>a+b,0)/(points.length||1));
  ...
  async function loadAnalytics(){
    ...
    try{
      ...
      renderList('aTopPages',d.topPages,0,0);renderList('aSources',d.trafficSources,0,1);renderList('aSections',(d.topSections&&d.topSections.length?d.topSections:d.topSectionIds),0,0);renderList('aButtons',d.topButtons,0,0);renderList('aNews',d.topNews,0,0);renderList('aCountries',d.countries,0,0);renderList('aDevices',d.devices,0,0);renderList('aScroll',d.scrollDepth,0,0);renderList('aRealtimePages',rt.topPages,0,0);
      ...
      const newsMode=health.preferredNewsDimension==='news_name'?'新闻名称':health.preferredNewsDimension==='news_type'?'新闻类型':'等待新闻自定义维度';
      warnings.textContent=custom.length?`GA4 事件已收到（${eventSummary}）。请建立 Event-scoped Custom definitions：${custom.map(x=>labels[x]||x).join('、')}。热门新闻当前使用：${newsMode}。${waitingItems.length?' '+waitingItems.join('；'):' '}`:waitingItems.length?`GA4 事件已收到（${eventSummary}）。热门新闻当前使用：${newsMode}。${waitingItems.join('；')}。新数据通常需数小时至 24 小时进入普通报表。`:diagnostics.length?`${diagnostics.length} 个非关键报表暂时不可用，其余 GA4 数据正常。事件：${eventSummary}`:`事件追踪正常：${eventSummary}`;
