(()=>{
  ...
  async function loadDataHealth(){
    ...
    const defs=[['news','News','/api/market-events'],['etf','ETF','/api/etf-engine'],['fed','Fed Rate','/api/rate-expectation-engine'],['gold','Central Bank Gold','/api/gold-reserves-engine']];
    const fetched=await Promise.all(defs.map(async([key,name,url])=>{try{const r=await fetch(`${url}?health=${Date.now()}`,{cache:'no-store'}),d=await r.json();return {key,name,url,ok:r.ok,data:d,error:r.ok?'':(d.error||`HTTP ${r.status}`)}}catch(e){return {key,name,url,ok:false,data:{},error:e.message}}}));
    const by=Object.fromEntries(fetched.map(x=>[x.key,x]));
    const news=by.news.data||{}, etf=by.etf.data||{}, fed=by.fed.data||{}, gold=by.gold.data||{};
    const nextEvent=(news.events||[]).filter(e=>new Date(e.datetime).getTime()>Date.now()).sort((a,b)=>new Date(a.datetime)-new Date(b.datetime))[0];
    const historyReady=(news.events||[]).filter(e=>Array.isArray(e.history)&&e.history.length>0).length;
    const newsChecked=news.lastCheckedAt||news.updatedAt, newsChanged=news.lastDataChangeAt||news.officialUpdatedAt;
    const newsTone=by.news.ok?(news.kvConfigured?'live':'warn'):'bad';
    setDcCard('dcNews',{status:toneLabel(newsTone,'Healthy','Warning','Offline'),tone:newsTone,main:`${(news.events||[]).length} events`,sub:nextEvent?`Next: ${nextEvent.name} · ${fmt(nextEvent.datetime)}`:'No upcoming event',details:[['Engine',news.engineVersion||'—'],['Last checked',fmt(newsChecked)],['Last data change',fmt(newsChanged)],['History available',`${historyReady}/${(news.events||[]).length}`],['Forecast filled',String((news.events||[]).filter(e=>e.forecast).length)],['Next event',nextEvent?`${nextEvent.name} · ${fmt(nextEvent.datetime)}`:'—']]});
    const etfChecked=etf.lastCheckedAt||etf.updatedAt, etfTone=by.etf.ok?(etf.sourceStatus==='live'?'live':freshTone(etfChecked,24*60)):'bad';
    setDcCard('dcEtf',{status:toneLabel(etfTone,'Live','Cached','Offline'),tone:etfTone,main:Number.isFinite(Number(etf.latestHoldings))?`${Number(etf.latestHoldings).toFixed(2)} t`:'—',sub:`Official date: ${etf.officialDate||'—'} · Change: ${Number(etf.dailyChange||0).toFixed(2)} t`,details:[['Engine',etf.engineVersion||'—'],['Source',etf.source||'SPDR'],['Official date',etf.officialDate||'—'],['Last checked',fmt(etfChecked)],['Last successful update',fmt(etf.lastSuccessfulUpdateAt||etf.updatedAt)],['KV write',String(etf.kvWrite===true?'Yes':'No')]]});
    const goldChecked=gold.lastCheckedAt||gold.checkedAt||gold.updatedAt, goldTone=by.gold.ok?(gold.sourceStatus==='live'?'live':freshTone(goldChecked,45*24*60)):'bad';
    setDcCard('dcGold',{status:toneLabel(goldTone,'Live','Cached','Offline'),tone:goldTone,main:`${(gold.records||[]).length} countries`,sub:`${gold.summary?.signal||'No signal'} · ${Number(gold.summary?.netMonthlyChangeTonnes||0).toFixed(2)} t`,details:[['Engine',gold.engineVersion||'—'],['Source mode',gold.sourceMode||'—'],['Last checked',fmt(goldChecked)],['Last successful update',fmt(gold.lastSuccessfulUpdateAt||gold.updatedAt)],['Countries',String((gold.records||[]).length)],['Warnings',(gold.errors||[]).length?gold.errors.join(' | '):'None']]});
    const fedChecked=fed.cmeLastCheckedAt||fed.lastCheckedAt||fed.updatedAt, fedManual=fed.sourceMode==='manual-admin-fallback', fedOfficial=fed.sourceMode==='official-github-sync'&&fed.live===true, fedTone=fedOfficial?'live':fedManual?'warn':by.fed.ok?'warn':'bad';
    const topOutcome=(fed.outcomes||[]).slice().sort((a,b)=>Number(b.probability)-Number(a.probability))[0];
    setDcCard('dcFed',{status:fedOfficial?'CME Live':fedManual?'Manual Active':by.fed.ok?'Fallback':'Offline',tone:fedTone,main:topOutcome?`${Number(topOutcome.probability).toFixed(1)}%`:'—',sub:topOutcome?`${topOutcome.targetRange} · ${topOutcome.move||topOutcome.direction||''}`:'No probability data',details:[['Engine',fed.engineVersion||'—'],['Effective source',fed.sourceMode||'—'],['Current target',fed.currentTargetRange||'—'],['Last CME check',fmt(fedChecked)],['Official fetch',fed.officialFetchSucceeded===true?'Succeeded':'Unavailable'],['Manual override',fed.manualOverrideAvailable?'Available':'Not saved']]});
    const tones=[newsTone,etfTone,goldTone,fedTone],scores=tones.map(t=>t==='live'?100:t==='warn'?70:0),health=Math.round(scores.reduce((a,b)=>a+b,0)/scores.length),fullChecks=[newsChecked,etfChecked,goldChecked,fedChecked].filter(Boolean).map(v=>new Date(v).getTime()).filter(Number.isFinite),lastFull=fullChecks.length?new Date(Math.min(...fullChecks)).toISOString():null;
    $('#dcOverallHealth').textContent=`${health}%`;const bar=$('#dcOverallBar');if(bar)bar.style.width=`${health}%`;$('#dcLastFullCheck').textContent=lastFull?mytClock(lastFull):'No record';$('#dcLastFullCheckAge').textContent=lastFull?agoText(lastFull):'No record';const badge=$('#overallBadge');badge.textContent=health>=90?'HEALTHY':health>=65?'ATTENTION':'ACTION NEEDED';badge.className=`badge ${health>=90?'ok':'warn'}`;
    $('#dcCloudflare').textContent=fetched.every(x=>x.ok)?'● Online':'▲ Partial';$('#dcKv').textContent=news.kvConfigured?'● Bound':'▲ Check binding';$('#dcForecast').textContent=news.kvConfigured?'● Ready':'▲ Unavailable';$('#dcSourceMode').textContent=fedOfficial?'CME Official':fedManual?'Fed Manual':'Mixed / Cached';$('#dcStatusLine').textContent=`Last dashboard check: ${fmt(now)} · Data pages remain read-only; this dashboard does not add KV writes.`;
    $('#dataHealthCards').innerHTML=fetched.map(x=>{const d=x.data||{},checked=d.lastCheckedAt||d.checkedAt||d.updatedAt;return `<div class="mini-row"><span>${x.ok?'●':'▲'} ${x.name}<small style="display:block;opacity:.65">Checked: ${checked?fmt(checked):'No timestamp'}</small><small style="display:block;opacity:.55">Engine: ${esc(d.engineVersion||'—')}</small></span><b style="color:${x.ok?'#83d79f':'#ef7777'}">${x.ok?'Available':'Offline'}</b></div>`}).join('');
  }
