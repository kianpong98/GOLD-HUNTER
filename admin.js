(()=>{
  ...
  async function loadDataHealth(){
    ...
    const rows=await Promise.all(apis.map(async([name,url])=>{try{const r=await fetch(`${url}?health=${Date.now()}`,{cache:'no-store'}),d=await r.json();return [name,r.ok,d.updatedAt||d.officialUpdatedAt||d.generatedAt||d.sourceUpdatedAt||null,r.ok?'Healthy':'Warning']}catch{return [name,false,null,'Offline']}}));
    $('#dataHealthCards').innerHTML=rows.map(([n,ok,t,status])=>`<div class="mini-row"><span>${ok?'●':'▲'} ${n}<small style="display:block;opacity:.65">${t?fmt(t):'No timestamp'}</small></span><b style="color:${ok?'#83d79f':'#efaa69'}">${status}</b></div>`).join('');
  }
